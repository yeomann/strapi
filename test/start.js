...
const databases = {
  ...
  postgres:
    '--dbclient=postgres --dbhost=127.0.0.1 --dbport=5432 --dbname=strapi-test --dbusername= --dbpassword=',
  mysql:
...
const main = async () => {
  ...
  const generate = database => {
    return new Promise((resolve, reject) => {
      const appCreation = spawn(
        'node',
        `${strapiBin} new ${appName} --dev ${database}`.split(' '),
        { detached: true },
      );
